...
public class Friends {
	...
	public static ArrayList<ArrayList<String>> cliques(Graph g, String school) {
		...
		for (int i = 0; i < g.members.length; i++) {
			...
			while (!visitQueue.isEmpty()) {
				...
				for (Friend frd = g.members[crnt].first; frd != null; frd = frd.next) {
					if (!visited[frd.fnum] && g.members[frd.fnum].student
							&& g.members[frd.fnum].school.equals(school)) {
						visited[frd.fnum] = true;
	 ...
	 *         connectors.
	 */
	public static ArrayList<String> connectors(Graph g) { // Has issues but does not require DFS
		...
		for (int i = 0; i < g.members.length; i++) {
			...
			if (numFriends == 1) {
				...
				if (firstNumFriends == 1
						&& g.members[g.members[g.members[i].first.fnum].first.fnum].name.equals(g.members[i].name)) {
					continue;
